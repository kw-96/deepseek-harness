...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  {
    ...
    kind: 'single',
    scope: 'session',
    summary: 'The bottom panel spans the center and details regions.',
    doc: 'The bottom panel spans the center and details regions. It is session\nscoped so terminal ownership follows the selected live Agent.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'bottom\', () => ctx.slots.register(\n      { name: \'bottom\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    kind: 'keyed',
    scope: 'root',
    summary: '官方 v0.1.5 中央面板槽在本地的别名槽：官方包（ui-conversation 的 ConversationPanel，key 恒为保留的 \'conversation\'；未来的全局面板按 各自 key）注册到这里，AppFrame 在中心列与本地 \'conversation\' 槽 同时渲染，并以 usePanelInfo 的 activePanelId（缺省 \'conversation\'） 作 keyed 分发。',
    doc: '官方 v0.1.5 中央面板槽在本地的别名槽：官方包（ui-conversation 的\nConversationPanel，key 恒为保留的 \'conversation\'；未来的全局面板按\n各自 key）注册到这里，AppFrame 在中心列与本地 \'conversation\' 槽\n同时渲染，并以 usePanelInfo 的 activePanelId（缺省 \'conversation\'）\n作 keyed 分发。',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: '官方 v0.1.5 右栏槽在本地的别名槽：官方包（ui-sidebar-right 的 RightbarRoot）注册到这里，AppFrame 在右列与本地 \'details\' 槽同时 渲染。保持官方的 root 作用域与 RightbarOwnerProps owner share （occupant 自带 SessionProvider 门控会话内容）；面板锚定帧右缘、 列关闭时滑出帧外，形态上报经 ctx.layout.openRightbar/closeRightbar 映射到本地 details 列的开合。',
    doc: '官方 v0.1.5 右栏槽在本地的别名槽：官方包（ui-sidebar-right 的\nRightbarRoot）注册到这里，AppFrame 在右列与本地 \'details\' 槽同时\n渲染。保持官方的 root 作用域与 RightbarOwnerProps owner share\n（occupant 自带 SessionProvider 门控会话内容）；面板锚定帧右缘、\n列关闭时滑出帧外，形态上报经 ctx.layout.openRightbar/closeRightbar\n映射到本地 details 列的开合。',
    registerOptions: [],
    ownerProps: [
