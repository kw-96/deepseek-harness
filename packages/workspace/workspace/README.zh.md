...
- **成员资格是记账，不是记录目录**——会话通过持有其 id 的 Workspace 记账归属项目；`attachSession` 只校验会话存在，因此记录在其它目录的会话可以被记入任意 Workspace（界面用目录挑选工作树，从不作为准入条件）。
- **外部变更延迟可见**——如果另一进程删除或损坏目录，项目只能在下次刷新或重启后反映出来。
- **归档可逆**——被隐藏的会话保留其历史与工作区位置，因此 `unarchiveSession` 会把它恢复到原位置；归档集合是持久的显示过滤器，而非删除。
