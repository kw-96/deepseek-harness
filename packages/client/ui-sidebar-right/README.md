...
Normal and fullscreen presentations share the same content tree, so switching does not remount tabs. The normal panel anchors to the right column; fullscreen covers the viewport while retaining the wide-screen columns underneath. A viewport that cannot spare a track for the panel — below 768px, or anywhere the frame's solved track bottoms out at zero while the conversation keeps its floor — opens in fullscreen automatically rather than closing on itself; leaving fullscreen where there is no track closes the panel, and widening does not reopen a closed panel. A fullscreen opening keeps the underlying columns unchanged until its slide finishes, then prepares the normal track without a column transition. Before a fullscreen panel retreats, closing prepares a full-width conversation and restoring prepares the normal right track; the background does not animate during the retreat.
