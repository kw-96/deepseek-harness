...
function resolveModuleFallbackEntries(
  ...
  unresolvedNames: ReadonlySet<string>
...
export async function healProfilesModuleFallback(
  ...
  const { entries, packageNames, unresolvedNames, packageDirs, declarers, versions } = resolveModuleFallbackEntries(installAnchor, materialize)
  if (materialize && moduleFallbackStale(modulesDir, entries, unresolvedNames)) {
