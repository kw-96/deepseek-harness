/**
 * dsh-codex-left 浏览器入口：挂载 codexLeft Remote（目录列举 + 项目注册表）
 * 并注册左侧导航面 ——
...
import remoteContribution from 'dsh-codex-left/remote'
...
export async function apply(ctx: Context): Promise<() => Promise<void>> {
  ...
  const disposeLocale = locale.register('codex-left', { zh, en } as Record<string, Record<string, string>>)
  ...
  const disposeBrandName = slots.inject('sidebar.brand.name', () => slots.register({
    name: 'sidebar.brand.name', id: 'codex-hide-brand-name', locale: 'codex-left',
  }, () => null))
  ...
  const disposeBrandMark = slots.inject('sidebar.brand.mark', () => slots.register({
    name: 'sidebar.brand.mark', id: 'codex-sidebar-brand-controls', locale: 'codex-left',
  ...
  const disposeHeroPicker = slots.inject('conversation.hero.workspace', () => slots.register({
    name: 'conversation.hero.workspace', priority: -1, locale: 'codex-left',
