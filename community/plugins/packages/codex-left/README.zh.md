...
DeepSeek Harness（DSH）的 Codex 式左侧导航栏：会话浏览器（带持久项目层）、添加工作区弹窗与新建会话页的项目选择器，**单插件组合**。会话菜单的「在终端中打开」用 `dsh-better-sidebar` 的工作台终端；自研底栏终端插件 `dsh-codex-shell` 已于 2026-09-15 从本仓库下线（源码保留在 git 历史里）。
...
会话菜单「打开方式 → 在终端中打开」为该会话打开 `dsh-better-sidebar` 的终端 tab——本部署当前用的就是它，一个会话一个终端实例。该插件是唯一支持的终端宿主；未安装时该项报错提示缺失插件，本插件其余功能不依赖它。
...
- 「在终端中打开」需要 `dsh-better-sidebar`；未安装时该项报错提示缺失插件。
