...
1. **"Connect GitHub" 页面**（作为 `settings.plugins.tab` 页面落在 dsh 设置 → 插件的标签栏，ADR-0008）：[连接 GitHub] 启动 Device Flow——用户码自动复制、授权页自动打开，进度按服务端节奏轮询 `deviceFlowStatus` 获得（`slow_down` 时拉长间隔）；已连接用户看到 `已连接 @login` 与 [断开连接]。token 永不经过前端。
