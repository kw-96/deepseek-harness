...
export function Graph(props: GraphProps): React.ReactNode {
  const {
    t, log, branch, busy, locateSignal, openCommit, detail, openFile, fileDiff, onOpenCommit, onCloseDetail,
    onToggleFile, onLocate, onFetch, onPull, onPush, onRefresh,
  } = props
              ...
              return (
                <div key={row.commit.hash}>
                  <div className={[
                    ...
                    <button type="button" className={css.commitMain}
                      title={`${t('openDetail')}：${row.commit.subject}`}
                      aria-label={`${t('openDetail')} ${row.commit.shortHash}`}
                      onClick={() => { open ? onCloseDetail() : onOpenCommit(row.commit.hash) }}>
                  ...
                  {open && detail !== null && (
                    <CommitDetail t={t} hash={row.commit.hash} state={detail} openFile={openFile}
                      fileDiff={fileDiff} onToggleFile={onToggleFile} onClose={onCloseDetail} />
                  )}
                </div>
              )
            })}
