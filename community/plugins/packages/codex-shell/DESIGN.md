...
插件自有装饰（置顶、未读、子代理、运行态）全部落在上述别名上，不引入
任何独立色值。

## 排版

- 搜索框输入 13px；工作区标题 13px / 600（弱化色、轻字距）
- 会话行标题 14px / 500 单行截断；子代理行 12.5px / 400；相对时间 11px
- 工作区与会话、会话与子代理形成三档字体层级
...
- 固定行高：工作区标题 26px、会话行 32px、子代理行 26px（超长一律省略号截断）
- 圆角：行 8px、搜索框与输入 10px、菜单 12px、弹窗 14px
