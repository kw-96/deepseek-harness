...
- **Visuals**: maps the host's `--dsw-*` theme tokens through the same `--cx-*` aliases `dsh-codex-left` uses, and follows light and dark themes automatically.
...
## Composing with dsh-codex-left

`dsh-codex-left`'s session menu calls `terminalOpen` on this plugin's `codexShell` Remote for its "Open in terminal" entry. The service name and the `terminalOpen(sessionId, options)` signature are the cross-plugin interface: rename either only together with the consumer. This plugin depends on nothing from `dsh-codex-left`.
