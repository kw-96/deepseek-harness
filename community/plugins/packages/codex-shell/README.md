...
- **右侧与底部工作台**：右侧 `details` 列提供文件树、Git 变更分组/差异/提交/时间线历史/分支、附加目录管理、插件管家、摘要和浏览器；底部独立终端使用当前会话的持久 pwsh PTY，支持命令、滚动输出、清空和关闭。列宽/拖拽/动画由宿主布局接管。
...
```sh
dsh plugin --profile web add dsh-codex-shell@0.4.4
```
