...
function mount(){if(document.querySelector('[data-workorder-entry]'))return;const settings=document.querySelector('[class*=settingsArea]');if(!settings)return;const link=document.createElement('a');link.dataset.workorderEntry='';link.href=path;link.textContent='${PRODUCT_NAME}';link.setAttribute('aria-label','打开 ${PRODUCT_NAME}');link.style.cssText='display:flex;align-items:center;justify-content:center;min-height:36px;margin:4px 0;padding:0 12px;border-radius:10px;color:inherit;text-decoration:none;font-size:14px;background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.1))';settings.parentElement?.insertBefore(link,settings)}
