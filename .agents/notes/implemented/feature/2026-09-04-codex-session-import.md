...
`@deepseek-ai/dsh-session-import-codex` exposes manual import by default. Its `autoSync` setting defaults to `false`; enabling it runs one import immediately and enables the configured periodic sweep. It reads Codex's `thread_history_1.sqlite` (`thread_items` and `thread_turns`) read-only through `node:sqlite` and joins the optional `session_index.jsonl` titles. Each thread converts into contiguous DSH events — `user/message`, `assistant/message`, and, per Codex tool item, that item's requesting assistant message followed by its `tool/call`/`tool/result` pair, plus `turn/start`/`turn/end`, `session/title`, and a trailing `session/end-seed` — with Codex timestamps and caps on tool-result and title text. The requesting assistant message carries the call id, name, and arguments the Codex item records, so every imported tool result answers a declared call and the transcript stays valid on every provider ([rationale](../bug-fix/2026-09-10-tool-call-pairing-in-imported-history.md)). Every thread maps to one fixed session id, `codex-<threadId>`. The `codexImport` Remote also serves `scan()`, `undo(at)`, and `restore(at)`: a read-only preview of the next sweep, and reversible archiving of the sessions one recorded run created ([undo and preview](2026-09-14-codex-import-undo-and-preview.md)).
