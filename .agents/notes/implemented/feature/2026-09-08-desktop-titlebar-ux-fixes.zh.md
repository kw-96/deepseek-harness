...
Codex 式桌面顶栏上线后暴露多处体验问题：图标与菜单文字偏小；菜单展开后点击顶栏空白处不会自动收起（只有点击顶栏之外才收起）；最大化按钮不随窗口状态切换图标与语义标签；顶栏按钮容器尺寸与高亮样式不统一。

## 决策

- **尺寸放大**：顶栏高度 36→40 像素，图标按钮 28→32 像素，面板 / 后退 / 前进图标 16/14→18/16 像素，窗口按钮图标 10→12 像素，菜单文字 13→14 像素。
- **按钮样式统一（Windows 原生）**：顶栏全部按钮统一为全高 40px、无圆角、紧贴排列的容器（图标按钮 40 宽、窗口按钮 46 宽、菜单按钮随文字宽），共用同一悬停高亮（整块灰色），移除图标按钮 0.85 的常驻透明度；关闭按钮悬停保留语义红色。曾先统一为 32×32 圆角方块，用户看后改选原生样式。
...
**按钮统一为 32×32 圆角方块（现代风）。** 先按此实现，用户看后选择 Windows 原生全高无圆角样式，已切换。
