...
**这里不做中断（已由后续笔记取代，仅针对运行中的行）。** [从会话头停止单个后台任务](2026-09-14-session-job-stop-control.zh.md)补上了这条笔记当年记下的悬而未决之处：命令自己向拥有该任务的 Agent 注入通知，因为 `kill()` 会把终态投递标为已上报，注册表因此不再发通知。
