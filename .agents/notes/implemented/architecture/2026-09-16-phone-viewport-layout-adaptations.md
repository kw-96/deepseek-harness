...
**A touch device also drops the resident rail.** The collapsed sidebar's 56px rail is a permanent column paying rent in width for controls used occasionally; on a phone it took 13% of the viewport in every state. A touch device therefore solves the sidebar track to zero whether the sidebar is collapsed or not, and reaches the sidebar through one entry: the same 28px circle around a 15px glyph as the right panel's own entry, sitting in the conversation header's leading corner with the mirrored inset. A tap opens the full sidebar — the icon column is a collapsed state, not a menu to walk through, and everything it offers (new session, SSH, workspace search, settings) is in the drawer. The entry renders while the header is hidden as well, for a blank or absent Session, because a refresh lands there and the session list is then the only way back into a conversation. The gate is `navigator.maxTouchPoints > 0`, for the same desktop-mode reason as the composer focus rule below. `SIDEBAR_COLLAPSED` stays the rendered width of the icon column, its one remaining consumer.

**The header keeps an inset free for that entry.** The frame publishes it as `--dsh-rail-entry-clearance` (48px: inset 12 + button 28 + gap 8) and the title row consumes it as a left margin. That margin is a base rule rather than a phone-breakpoint one because the entry follows touch input, not viewport width: a phone browser in desktop mode is 980px wide and still gets the button. The entry stays in the frame, which already owns the sidebar geometry, instead of registering into a conversation header slot — a slot would give `ui-layout` a type dependency on `ui-conversation` that already depends on it in the other direction, and project references reject the cycle.
