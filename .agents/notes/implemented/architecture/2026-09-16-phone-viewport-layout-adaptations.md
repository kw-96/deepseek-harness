...
**That button lands in the conversation header's corner, and the header yields the space.** The button sits where the header's title row draws, so an inset is needed or the session title runs under it. The frame publishes the inset as `--dsh-rail-entry-clearance` (60px: inset 8 + button 44 + gap 8) and the title row consumes it as a left margin. That margin is a base rule rather than a phone-breakpoint one because the entry follows touch input, not viewport width: a phone browser in desktop mode is 980px wide and still gets the button. The entry stays in the frame, which already owns the sidebar geometry, instead of registering into a conversation header slot — a slot would give `ui-layout` a type dependency on `ui-conversation` that already depends on it in the other direction, and project references reject the cycle. The button keeps a 44px touch target where a header control would have been 28px.
